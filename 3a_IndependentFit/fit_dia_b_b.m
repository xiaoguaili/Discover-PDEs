...
%independently. This script is to fit diagonal component.
...
%expanded on \rho_b and \nabla \rho_b.
%In this script, only rho_b_b, drho_b_b, diagonal_b_b are used.

fitID=22; %fit options

%% fit diagonal term
[xData, yData, zData] = prepareSurfaceData(rho_b_b, drho_b_b, diagonal_b_b);
...
zlabel('$\langle K_{\rho_{b}+\nabla \rho_b(x-x_b)} \gamma_b, \gamma_{b} \rangle$','Interpreter','latex','FontSize',20);
mkdir('figures')
% savefig('figures/dia_b_b.fig')
% save(['dia_fit_',num2str(fitID),'.mat'],'fitresult')
...
ylabel('$\nabla \rho$','Interpreter','latex','FontSize',20)
xlabel('$\rho$','Interpreter','latex','FontSize',20)
zlabel('error of $\langle K_{\rho_{b}+\nabla \rho_{b}(x-x_b)} \gamma_b, \gamma_{b} \rangle\ [\%]$','Interpreter','latex','FontSize',20);
set(gca,'FontSize',20);
h.EdgeColor = 'none';
box on
clear h 
mkdir('errorPlot')
% savefig(['errorPlot/error_dia_',num2str(fitID),'.fig'])