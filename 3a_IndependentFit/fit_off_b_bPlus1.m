...
%expanded on \rho_b and \nabla \rho_b.
%In this script, only rho_b_bPlus1, drho_b_bPlus1, off_b_bPlus1 are used.

fitID=22; %fit options

%% fit offdiagonal term
[xData, yData, zData] = prepareSurfaceData(rho_b_bPlus1, drho_b_bPlus1, off_b_bPlus1);
...
zlabel('$\langle K_{\rho_{b}+\nabla \rho_b(x-x_b)} \gamma_b, \gamma_{b+1} \rangle$','Interpreter','latex','FontSize',20);
mkdir('figures');
savefig('figures/off_b_bPlus1.fig')
save(['off_b_bPlus1_fit_',num2str(fitID),'.mat'],'fitresult')
...
ylabel('$\nabla \rho$','Interpreter','latex','FontSize',20)
xlabel('$\rho$','Interpreter','latex','FontSize',20)
zlabel('error of $\langle K_{\rho_{b}+\nabla \rho_{b}(x-x_b)} \gamma_b, \gamma_{b+1} \rangle\ [\%]$','Interpreter','latex','FontSize',20);
set(gca,'FontSize',20);
h.EdgeColor = 'none';
box on
clear h 
mkdir('errorPlot')
savefig(['errorPlot/error_off_b_bPlus1_',num2str(fitID),'.fig'])